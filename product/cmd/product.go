...
func RunProductService(c context.Context) {
	...
	logger := log.InitLogger(fmt.Sprintf("/var/log/%s.log", constants.APP_PRODUCT_SERVICE)).
		With().
		Str(log.KEY_APP_NAME, constants.APP_PRODUCT_SERVICE).
		Str(log.KEY_TAG, "main RunProductService").
		Logger()

	logger = logger.With().Str(log.KEY_PROCESS, "initializing config").Logger()
	logger.Info().Msg("initializing config")
	c = logger.WithContext(c)
	cfg := config.InitConfig(c, constants.APP_PRODUCT_SERVICE)
	...
	c = logger.WithContext(c)
	shutdownFuncs, err := otel.InitOtelSdk(c, constants.APP_PRODUCT_SERVICE, cfg.Otel)
	if err != nil {
	...
	defer func() {
		logger := logger.With().Str(log.KEY_PROCESS, "shutting down database connection").Logger()
	...
	defer func() {
		logger := logger.With().Str(log.KEY_PROCESS, "shutting down cache connection").Logger()
	...
	logger = logger.With().Str(log.KEY_PROCESS, "initializing productService").Logger()
	...
	logger = logger.With().Str(log.KEY_PROCESS, "attach product controller").Logger()
